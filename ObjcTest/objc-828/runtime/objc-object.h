...
objc_object::getIsa() 
{
    //非TaggedPointer对象 获取isa
    if (fastpath(!isTaggedPointer())) return ISA();

    //从TaggedPointer对象指针中获取类
...
#else
        //2. 为bits赋默认值 0x001d800000000001ULL
        newisa.bits = ISA_MAGIC_VALUE;
        // isa.magic is part of ISA_MAGIC_VALUE
        // isa.nonpointer is part of ISA_MAGIC_VALUE
        
        //ISA_HAS_CXX_DTOR_BIT 为 1
...
objc_object::rootRetain(bool tryRetain, objc_object::RRVariant variant)
{
    ...
    if (variant == RRVariant::FastOrMsgSend) {
        ...
        // 它们在这里是为了避免重新加载 isa
        // hasCustomRR()方法用于判定对象是否有自定义的retain函数指针
    ...
    do {
        ...
        //如果不是nonpointer-isa, 则引用计数只存在于引用计数表中
        if (slowpath(!newisa.nonpointer)) {
            ClearExclusive(&isa.bits);
            //如果是try操作, 则操作后直接返回
            if (tryRetain) return sidetable_tryRetain() ? (id)this : nil;
            
            //直接操作引用计数表, 引用计数表的计数+1, 执行完直接return, 不再继续向下
        ...
        //执行到此 说明是nonpointer-isa, 操作extra_rc+1
        //carry 标志extra_rc 是否超过最大值
        uintptr_t carry;
         ...
         */
        //addc(): newisa.bits + RC_ONE + 0 若64bit上溢出则在输出指针carryout中写入1，不溢出则carry为0
        //extra_rc 存储在高位, 占据64-45的位置,newisa.bits+RC_ONE, 即相当于extra_rc+1
        //之所以每次递增的量是RC_ONE取决于extra_rc在isa.bits中的位域, newisa.bits+RC_ON等价于newisa.bits.extra_rc += 1
        //extra_rc产生溢出时的值为 1<<19  十进制: 524288 16进制:x20000
        newisa.bits = addc(newisa.bits, RC_ONE, 0, &carry);  // extra_rc++

        //如果extra_rc超过最大值, 就把它赋值为最大值的一半, 另一半存到SideTable里
        if (slowpath(carry)) {
            // newisa.extra_rc++ overflowed
            if (variant != RRVariant::Full) {
                ClearExclusive(&isa.bits);
                //rootRetain_overflow -> rootRetain (variant = RRVariant::Full)
                //调用rootRetain_overflow 然后内部再调用rootRetain只不过此时的variant参数变为Full了
            ...
            //extra_rc的一半即是 1<<18 十进制: 262144   16进制:0x40000
            //设置对象的isa.bits.extra_rc为RC_HALF，也就是保留一半的引用计数在extra_rc位域；
            newisa.extra_rc = RC_HALF;
            //设置对象的isa.bits.has_sidetable_rc位为1，表示使用 SideTable 协同保存对象引用计数
            newisa.has_sidetable_rc = true;
        }
    //StoreExclusive 比较oldisa.bits和isa.bits的值，若两者相等则将newisa.bits写入&isa.bits指针的内容且返回true，否则不写入且返回false；
    } while (slowpath(!StoreExclusive(&isa.bits, &oldisa.bits, newisa.bits)));
...
ALWAYS_INLINE bool //返回值布尔累心, 表示引用计数是否下溢出
objc_object::rootRelease(bool performDealloc, objc_object::RRVariant variant)
{ //performDealloc参数指定引用计数下溢出时 是否需要向对象发送SEL_dealloc消息
...
retry:
    do {
        newisa = oldisa;
        //指针类型的isa时，直接走 SideTable 的引用计数递减操作
        if (slowpath(!newisa.nonpointer)) {
            ClearExclusive(&isa.bits);
 ...
 underflow:
    ...
    if (slowpath(newisa.has_sidetable_rc)) {
        ...
        // sidetable中的引用计数减少RC_HALF
        // borrow为取出的引用计数数量 可能是RC_HALF也可能是0
        auto borrow = sidetable_subExtraRC_nolock(RC_HALF);

        //borrow.remaining == 0 表示sidetable空了, 里面的引用计数为0了
        ...
        if (borrow.borrowed > 0) {
            ...
            // sidetable里也是空的,说明引用计数为0了 该向下执行销毁对象了
        }
    }
