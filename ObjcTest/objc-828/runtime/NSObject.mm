...
objc_object::sidetable_addExtraRC_nolock(size_t delta_rc)
{
    ...
    //也就是上溢出后,refcntStorage 仍为允许的最大值 1111....111111  62位
    if (carry) {
        //#define WORD_BITS                    64
        //#define SIDE_TABLE_RC_PINNED         (1UL<<(64-1))    1<<63
...
{
    //sidetable_retain ()只针对使用指针类型的isa的对象
#if SUPPORT_NONPOINTER_ISA
    ASSERT(!isa.nonpointer);
#endif
    ...
    // SIDE_TABLE_RC_PINNED = 1<<63
    // 判断refcntStorage的最高位为0，仅最高位为0才递增refcntStorage
    if (! (refcntStorage & SIDE_TABLE_RC_PINNED)) {
        //引用计数+1
        //#define SIDE_TABLE_RC_ONE            (1UL<<2)
        //指针isa模式下SideTable中保存数据的位域机构和isa.bits不一样：SideTable中从最低位起第1位SIDE_TABLE_WEAKLY_REFERENCED标记是否弱引用，第2位SIDE_TABLE_DEALLOCATING标记对象是否已析构，从第3位开始保存引用计数refCount，因此内存计数递增使用的增量是SIDE_TABLE_RC_ONE即0x04；
...
{
    //预编译判断如果支持非指针类型isa，则禁止使用非指针类型isa的对象执行tryRetain,即tryRetain只针对使用指针类型的isa的对象
#if SUPPORT_NONPOINTER_ISA
    ASSERT(!isa.nonpointer);
#endif
    //调用SideTables()静态方法，获取管理SideTable的StripedMap，
    //再以对象this为关键字获取对象所对应的SideTable 记为table
    SideTable& table = SideTables()[this];
    ...
    // }
    // _objc_rootTryRetain()只会被_objc_loadWeak()方法调用， 而_objc_loadWeak()方法内的逻辑已经获取了SideTable的锁，因此不必在_objc_rootTryRetain()中重复加锁；
