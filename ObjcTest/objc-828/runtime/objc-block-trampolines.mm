...
_imp_implementationWithBlock_init(void)
{
    ...
    // 在某些进程中渴望加载libobjc-trampolines.dylib。
    // 一些程序（最著名的是嵌入式Chromium的较早版本使用的QtWebEngineProcess） 启用了严格限制的沙箱配置文件，从而阻止了对该dylib的访问。
    //  如果有任何调用imp_implementationWithBlock 的操作（如AppKit开始执行的操作） ，那么我们将在尝试加载它时崩溃。 将其加载到此处可在启用沙箱配置文件之前对其进行设置并阻止它。
