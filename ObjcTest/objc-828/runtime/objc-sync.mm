...
/*
 这个结构体包含一个 object（嗯就是我们给 @synchronized 传入的那个对象）和一个有关联的 recursive_mutex_t，它就是那个跟 object 关联在一起的锁。每个 SyncData 也包含一个指向另一个 SyncData 对象的指针，叫做 nextData，所以你可以把每个 SyncData 结构体看做是链表中的一个元素。最后，每个 SyncData 包含一个 threadCount，这个 SyncData 对象中的锁会被一些线程使用或等待，threadCount 就是此时这些线程的数量。它很有用处，因为 SyncData 结构体会被缓存，threadCount==0 就暗示了这个 SyncData 实例可以被复用。
 */
...
///先是哈希出对象的数组下标，然后取出数组对应元素的 lock
#define LOCK_FOR_OBJ(obj) sDataLists[obj].lock
///先是哈希出对象的数组下标，然后取出数组对应元素的 data
...
static SyncData* id2data(id object, enum usage why)
{
    ...
    //执行到这儿,说明没有在线程空间 和 缓存中找到, 需要自己开辟空间 创建一个新的SyncData结构, 并为它赋值
