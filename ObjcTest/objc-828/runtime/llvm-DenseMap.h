...
public:
  ...
  // The value is constructed in-place if the key is not in the map, otherwise
  // it is not moved.
  //如果键不在map中，则将键、值对插入到map中。
  //如果key不在map中，则会在适当位置构造该值，否则不会移动该值。
  template <typename... Ts>
...
private:
  ...
  /// returns false.
  /// Val 查找合适的桶，在 FoundBucket 中返回。 如果存储桶包含键和值，则返回 true，否则返回带有空标记或墓碑的存储桶并返回 false。
  template<typename LookupKeyT>
  bool LookupBucketFor(const LookupKeyT &Val,
    ...
    while (true) {
      ...
      // key即地址相同,则找到了对应的桶 放入FoundBucket 返回true
      ...
      if (LLVM_LIKELY(KeyInfoT::isEqual(ThisBucket->getFirst(), EmptyKey))) {
        ...
        // 如果我们在探测时已经看到了墓碑，请将其填入，而不是我们最终探测到的空桶。
      ...
      // 如果这是一个墓碑，记住它。 如果 Val 最终不在map中，我们宁愿返回它而不是需要更多探测的东西。 零值同上。
