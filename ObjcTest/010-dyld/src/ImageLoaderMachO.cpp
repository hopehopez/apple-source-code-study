...
// 确定这个mach-o文件是否有经典的或压缩的LINKEDIT，以及它有多少段
...
// 为主可执行文件创建镜像
// 返回一个ImageLoader类型的image对象，即主程序
// 其中sniffLoadCommands函数 获取Mach-O类型文件的Load Command的相关信息，并对其进行各种校验
ImageLoader* ImageLoaderMachO::instantiateMainExecutable(const macho_header* mh, uintptr_t slide, const char* path, const LinkContext& context)
{
	...
	// sniffLoadCommands方法用于读取此mach-o文件是否具有压缩,段数,lib数,
...
//核心主要是for循环加载方法的调用，这里需要注意的一点是，libSystem的初始化必须先运行
