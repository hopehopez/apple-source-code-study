...
void initializeMainExecutable()
{
	...
	// 运行主可执行文件的初始值设定项和它所带来的一切
	sMainExecutable->runInitializers(gLinkContext, initializerTimes[0]);
	
	// register cxa_atexit() handler to run static terminators in all loaded images when this process exits
	// 注册 cxa_atexit()处理程序，以便在此进程退出时在所有加载的映像中运行静态终止符
...
// 为了安全起见，setuid程序忽略DYLD_*环境变量。
// 此外，DYLD_*environment变量将从环境中删除，以便任何子进程都看不到它们。
...
// 在dyld 获得控制之前, 内核会映射到主可执行文件中.
// 我们需要为已经映射到主可执行文件中的文件,创建一个ImageLoader*对象.
...
void registerObjCNotifiers(_dyld_objc_notify_mapped mapped, _dyld_objc_notify_init init, _dyld_objc_notify_unmapped unmapped)
{
	...
	// 在libSystem初始化之前, 回调函数为空, lib加载初始化都不会通知objc, 所以为回调函数赋值以后, 扫描一下sAllImages, 将已加载初始化的通知objc
	for (std::vector<ImageLoader*>::iterator it=sAllImages.begin(); it != sAllImages.end(); it++) {
		ImageLoader* image = *it;
...
/*
 dyld的入口点。
 内核加载dyld并跳转到 __dyld_start，__dyld_start会设置一些寄存器并调用此函数。
 从__dyld_start跳转到该函数地址, 此函数返回目标程序中的main函数的地址,
 */
uintptr_t
		...
		uintptr_t* startGlue)
{
	...
	// 主机的结构中可能没有平台字段，肯定留有空间用以填充它，因此始终设置它
	{
	...
	// 如果这是主机dyld，检查iOS模拟器是否正在运行
	const char* rootPath = _simple_getenv(envp, "DYLD_ROOT_PATH");
	if ( (rootPath != NULL) ) {
		// look to see if simulator has its own dyld
		// 查看模拟器是否有自己的dyld
		// 如果模拟器有自己的dyld, 就用模拟器的dyld, 并返回result, 结束函数执行
	...
	// 检查我们是否应该强制dyld3。注意，由于AMFI，我们必须在常规环境解析之外进行此操作
	if ( dyld3::internalInstall() ) {
    ...
    //非mac端操作
	{
		...
		//检查设置的环境变量 为环境变量sEnv的变量赋值
		checkEnvironmentVariables(envp);
		//如果DYLD_FALLBACK_FRAMEWORK_PATH为空, 将其设置为默认值
	...
	// 在常规逻辑之外解析这个环境变量，因为我们希望在没有实体元素的二进制文件上接受它
#if !TARGET_OS_SIMULATOR
	if ( _simple_getenv(envp, "DYLD_JUST_BUILD_CLOSURE") != nullptr ) {
		...
		// 如果数据包含的环境变量看起来有误，则不希望启动应用程序，因为那样会弹出UI
	...
	if ( sJustBuildClosure )
		sClosureMode = ClosureMode::On; //开启闭包模式
	...
	if ( gLinkContext.sharedRegionMode != ImageLoader::kDontUseSharedRegion ) {
		...
		// 如果此进程需要与共享区域不同的_DATA_CONST状态，请立即覆盖该状态
	...
	if ( getpid() == 1 ) {
		...
		//如果我们是PID 1（launchd）并且在macOS上，那么我们应该检查模拟器是否支持dylibs根。
		//如果它们在启动时不是根，并且文件系统是只读的，那么我们可以确定它们以后不会是根
...
//	将根检查器设置为comm页面中的状态
	...
	// 如果我们还没有一个闭包模式，那么检查环境和缓存类型, 设置闭包模式
...
#if !TARGET_OS_SIMULATOR
	if ( sClosureMode == ClosureMode::Off ) { //闭包模式关闭
		if ( gLinkContext.verboseWarnings )
			dyld::log("dyld: not using closures\n");
	} else { // 闭包模式开启 尝试以闭包方式启动程序
		...
		// 我们只想在运行时尝试构建一个闭包，如果它是iOS第三方二进制文件，或者是来自共享缓存的 macOS二进制文件
		...
		// 如果sJustBuildClosure为true, 在构建闭包后退出dyld，而不运行程序
		if ( sJustBuildClosure )
			_exit(EXIT_SUCCESS);
...
reloadAllImages:
		...
		// 实例化主可执行文件的ImageLoader
		
		// 3.3 主程序的初始化: 调用instantiateFromLoadedImage函数实例化了一个ImageLoader对象
		...
		// dyld_all_image_infos 镜像列表里并不包含dyld, 将dyld的路径加入到dyld_all_image_infos
#if TARGET_OS_SIMULATOR
		// get path of host dyld from table of syscall vectors in host dyld
		void* addressInDyld = gSyscallHelpers;
#else
		// get path of dyld itself
		// __dso_handle dyld二进制文件的链接符号
		...
		if ( len > 0 ) {
			...
			// strdup（）函数是c语言中常用的一种字符串拷贝库函数, 将字符串拷贝到新建的位置处
			if ( strcmp(dyldPathBuffer, gProcessInfo->dyldPath) != 0 )
				gProcessInfo->dyldPath = strdup(dyldPathBuffer);
		}

		// load any inserted libraries
		// 3.4 加载插入动态库: 遍历DYLD_INSERT_LIBRARIES环境变量，调用loadInsertedDylib加载
		...
		// 链接主程序之后执行此操作, 这样插入的动态库(例如: libSystem)不会在程序使用的动态库前面
		if ( sInsertedDylibCount > 0 ) {
		...
		// 3.9 寻找主程序入口即main函数：从Load Command 读取LC_MAIN入口，如果没有，就读取LC_UNIXTHREAD，这样就来到了日常开发中熟悉的main函数了
		{
			...
			if ( result != 0 ) {
				...
				// 主可执行文件使用LC_MAIN, 我们需要在libdyld中使用helper来调用mian()
				...
				// 主可执行文件使用LC_UNIXTHREAD, dyld需要在为mian()设置的程序中让"启动"
