...
#if __LP64__
typedef uint32_t mask_t;  // x86_64 & arm64 asm are less efficient with 16-bits  x86_64 和 arm64 asm 的 16 位效率较低
...
private:
    ...
    // 签名 newImp，使用 ＆_imp、newSel 和 cls 作修改。
...
#error Unknown method cache IMP encoding.
    ...
    //模板 set 函数 的声明，实现代码在 objc-cache.mm 中
...
private:
    ...
    // 高 16 位是 mask，即 _bucketsAndMaybeMask 右移 48 位得到 mask
    static constexpr uintptr_t maskShift = 48;
    ...
    // 可以保存的最大的 mask 值。(64 - maskShiift) 即掩码位数，然后 1 左移掩码位数后再 减 1 即 16 位能保存的最大二进制值
    static constexpr uintptr_t maxMask = ((uintptr_t)1 << (64 - maskShift)) - 1;
    
    // The mask applied to `_maskAndBuckets` to retrieve the buckets pointer.
    // 应用于 _maskAndBuckets 的掩码，以获取 buckets 指针。
    // 1 左移 44(48 - 4) 位后再 减 1（低44位全是1，其余都是 0 的数值）
...
//#define FAST_CACHE_ALLOC_DELTA16      0x0008
    ...
    bool hasFastInstanceSize(size_t extra) const
    {
        if (__builtin_constant_p(extra) && extra == 0) {
            // 如果 extra 在编译时是常量且值为 0，则 _flags & 0b0001 1111 1111 0000 的值返回，
    ...
    size_t fastInstanceSize(size_t extra) const
    {
        ...
        if (__builtin_constant_p(extra) && extra == 0) {
            // 如果 extra 在编译时是常量且值为 0，则直接返回 _flags & 0b0001 1111 1111 0000 的值
            return _flags & FAST_CACHE_ALLOC_MASK16;
        } else {
            size_t size = _flags & FAST_CACHE_ALLOC_MASK;
            // remove the FAST_CACHE_ALLOC_DELTA16 that was added
            // by setFastInstanceSize
            // 移除由 setFastInstanceSize 添加的 FAST_CACHE_ALLOC_DELTA16
            // 16字节对齐
...
struct protocol_t : objc_object {
    ...
    // 以下字段并不一定会存在 可以根据size > 8 * 指针大小 +  2 * sizeof(uint32_t) 来判断 是否存在额外的字段
