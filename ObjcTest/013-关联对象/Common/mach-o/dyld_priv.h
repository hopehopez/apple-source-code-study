...
//
// 仅供objc运行时使用
// 注册 处理程序，以便在映射、取消映射和初始化objc图像时调用, 注册回调函数
// dyld将会通过一个包含objc-image-info的镜像文件的数组回调mapped函数
// 这些dylibs的引用计数将自动增加，因此objc不再需要对它们调用dlopen（）来防止它们被卸载。
// 在调用_dyld_objc_notify_register()的过程中, dyld将使用已加载的objc 镜像 调用“mapped”函数。在以后的任何dlopen（）调用中，dyld还将调用“mapped”函数。
// 当Dyld在该镜像中被调用为初始化函数时，Dyld将调用“init”函数。这也是objc在该映像中调用+load方法的时候。
//
