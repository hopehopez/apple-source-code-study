                  ...
                  const struct mach_header * const mhdrs[])
{
    ...
    //查找具有Objective-C元数据的所有镜像。
    hCount = 0;

    // Count classes. Size various table based on the total.
    // 计算类的总数, 根据总数来调整各种表的大小.
    int totalClasses = 0;
    int unoptimizedTotalClasses = 0;
    {
        ...
        while (i--) {
            ...
            /// 在这里计算出totalClasses 和 unoptimizedTotalClasses
    ...
    /// 执行一次性运行时初始化，该初始化必须推迟到找到可执行文件本身。这需要在进一步初始化之前完成。
    /// 如果可执行文件不包含Objective-C代码，但随后动态加载Objective-C，则可执行文件可能不在此信息列表中。
    if (firstTime) {
...
void _objc_init(void)
{
    ...
    tls_init();
    //运行C++静态构造函数（只会运行系统级别的构造函数），在dyld调用静态析构函数之前，libc会调用_objc_init()
    static_init();
    //runtime运行时环境初始化，里面主要是unattachedCategories(分类初始化)、allocatedClasses（类的表初始化）
    runtime_init();
