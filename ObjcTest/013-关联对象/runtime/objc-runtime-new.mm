...
* Locking: runtimeLock must be held by the caller.
**********************************************************************/
/// 将类添加到所有类的表中。如果addMeta为true，则也会自动添加类的元类。
static void
addClassTableEntry(Class cls, bool addMeta = true)
{
    ...
    /// 通过共享缓存或数据段，该类可以是已知的类，但不允许它已经存在于动态表中。
    auto &set = objc::allocatedClasses.get();
...
static void moveIvars(class_ro_t *ro, uint32_t superSize)
{
    ...
    //先获取父类的instanceStart 成员变量起始位置偏移量
...
static void reconcileInstanceVariables(Class cls, Class supercls, const class_ro_t*& ro) 
{
    ...
    */
    ...
    if (ro->instanceStart >= super_ro->instanceSize) {// 父类的实例大小没有大于当前子类的实例大小,不做操作
        // Superclass has not overgrown its space. We're done here.
        return;
    }
    // fixme can optimize for "class has no new ivars", etc

    if (ro->instanceStart < super_ro->instanceSize) {// 父类的实例大小超过当前子类的实例大小, 要对当前子类的变量进行位移
...
static Class realizeClassWithoutSwift(Class cls, Class previously)
{
    ...
    //当类没有实现完成时 data 函数返回的是 class_ro_t
    //class_ro_t 中的内容都来自于 class 的定义中
    ...
    if (ro->flags & RO_FUTURE) {
        ...
        //将rw指针赋值给cls中的bits中,
        //当类实现完成时,data 函数返回的是 class_rw_t
        cls->setData(rw);
    }
    ...
    /// 如果cls的父类没有完成实现的话,就去实现父类
    supercls = realizeClassWithoutSwift(remapClass(cls->getSuperclass()), nil);
    /// 如果cls的元类没完成实现的话,就去实现元类
...
**********************************************************************/
///从headerList开始，对链接列表中的头文件执行初始处理。
...
# endif
    ...
    for (EACH_HEADER) {
        ...
        for (i = 0; i < count; i++) {
            /// 数组中会取出OS_dispatch_queue_concurrent、OS_xpc_object、NSRunloop 等系统类，例如CF、Fundation、libdispatch中的类。以及自己创建的类
            Class cls = (Class)classlist[i];
            /// 通过readClass函数获取处理后的新类
            Class newCls = readClass(cls, headerIsBundle, headerIsPreoptimized);
            
            
            const class_ro_t *ro = (class_ro_t *)cls->data();
            const char *cname = ro->getName();
            const char *oname = "ZPerson";
...
//                printf("_getObjc2ClassList 类名: %s - %p\n",cname, cls);
    ...
    for (EACH_HEADER) {
        ...
        // loaded later.
        /// 在启动时，我们知道预优化的映像引用指向协议的共享缓存定义。 我们可以跳过启动时的检查，但对于稍后加载的共享缓存映像，必须访问@protocol refs。
    ...
    ///5 分类处理
    ///加载分类。只有在完成分类附件初始化后才能执行此操作。
    ///对于启动时出现的分类，将推迟到调用_dyld_objc_notify_register完成后的第一次调用load_images时, 才执行加载分类。
...
static void resolveInstanceMethod(id inst, SEL sel, Class cls)
{
    ...
    //resolveInstanceMethod: 是一个类方法, 可在子类中自定义实现, 动态添加方法
    SEL resolve_sel = @selector(resolveInstanceMethod:);

    //查找resolveInstanceMethod: 的实现,
    //有就会缓存起来,后面调用时可以快速查找
    //没有就直接return
 ...
 *inst是cls或子类的实例，如果不知道，则为nil。

 *cls是要初始化和实现的类。

 *初始化器为true将初始化该类，为false将跳过初始化。
**********************************************************************/
static Class
...
///查找imp 找不回会返回转发的方法实现 _objc_msgForward_impcache
...
NEVER_INLINE
///慢速查找imp 或者 消息转发
//从快速查找转入慢速查找时 behavior=3
//方法决议后 在进行方法查找时 behavior=1
IMP lookUpImpOrForward(id inst, SEL sel, Class cls, int behavior)
{
    ...
    if (slowpath(!cls->isInitialized())) {
        ...
        //发送给类的第一条消息通常是+ new或+ alloc，或+ self，它通过objc_opt_ *或各种优化的入口点。

        //但是，该类目前尚未实现/初始化，并且优化的入口点通过objc_msgSend开始，最终在此处结束。
        
        //我们确实要避免对它们进行缓存，因为这可能导致IMP缓存永远由单个条目构成。

        //请注意，这种检查是明智的，因为多个线程可能会尝试同时第一次向给定的类发送消息，在这种情况下，我们仍然可能会缓存。
        
        // |= 与运算 并将结果进行赋值
        behavior |= LOOKUP_NOCACHE;  //behavior = 0b1011
    }

    // runtimeLock is held during isRealized and isInitialized checking
    // to prevent races against concurrent realization.
    // runtimeLock在isRealized和isInitialized检查期间保持，以防止与并发实现竞争。
    ...
    // runtimeLock 在方法查找和缓存期间保持, 以使方法方法添加保持原子性.
    ...
    //我们不希望人们能够制作看起来像类但实际上不是一个类的二进制文件并进行CFI攻击。
    //为了使这些操作更困难，我们希望确保该类是内置在二进制文件中或通过objc_duplicateClass，objc_initializeClassPair或objc_allocateClassPair合法注册的类。
    //2.2 检查是否是已知类 不是就报错 中断执行
    checkIsKnownClass(cls);

    //2.3
    //检查类是否已实现, 没有就实现一下  目的是为了确定父类链，方法后续的循环
    //检查类是否已初始化, 没有就进行初始化
    ...
    /*4. for 循环负责遍历查找 根据sel找到imp
     ...
     2. 在curClass(父类)的cache中查找, 缓存中是否有imp
        ...
        2.3 imp为空, curClass(父类)的缓存中没有没有找到, 继续循环, 去curClass(父类)的方法列表里查找, 如果还没找到就去curClass(父类)的父类(父类的父类)的缓存中查找, 如此循环往复, 结束结束
    ...
    */
    ...
    for (unsigned attempts = unreasonableClassCount();;) {
        if (curClass->cache.isConstantOptimizedCache(/* strict */true)) {
            ...
            // 从当前类的方法列表中查找, 二分查找算法，如果找到，则返回，将方法缓存到cache中
        ...
        if (slowpath(imp == forward_imp)) {
            ...
            // 如果在父类中找到了forward，则停止查找，且不缓存
            // 首先调用此类的方法解析器
            break;
        }
        if (fastpath(imp)) {
            // Found the method in a superclass. Cache it in this class.
            // 如果在父类中找到了此方法，将其存储到cache中
            goto done;
        }
    }
    ...
    /* 5. 执行到此, 没有找到方法实现，尝试一次方法决议, 并将resolveMethod_locked的执行结果返回
     
     说是一次, 是因为从MethodTableLookup汇编代码中,调用lookUpImpOrForward方法时,behavior=3
     进到if 里后, 会对behavior执行异或操作,behavior=1
     在resolveMethod_locked方法决议时,会再次尝试调用lookUpImpOrForward, 这时behavior=1, 不能再去if里去执行了.将去执行done:
     */
    ...
    /* 执行到此有两种可能:
     ...
     2. 通过上面的for循环,没有找到方法的实现,进行方法决议, 在方法决议时会再次执行lookUpImpOrForward方法,
     仍然没有找到方法实现imp. 前面说过,方法决议只会执行一次, 所以不会再去方法决议了, 而是向下执行
...
// 根据 objc_tag_index_t 返回 Class 指针
...
* memory.
 tagged pointer 混淆器旨在使攻击者更难将一个特定对象构造为 tagged pointer, 在缓冲区溢出或对某些内存进行其他写控制的情况下。
...
#endif
//        printf("tagged pointers 混淆值为: %lx\n", objc_debug_taggedpointer_obfuscator);
    } else {
...
* Returns nil if no class is using that tag or the tag is out of range.
 根据给定的tagged pointer tag 返回 一个Class
**********************************************************************/
Class
