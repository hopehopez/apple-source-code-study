 ...
 * 标志 调度块对象（dispatch block object）在提交给 DISPATCH_QUEUE_CONCURRENT 队列时应充当屏障块（barrier block）。
 * 有关详细信息，参考 dispatch_barrier_async。
 * 当直接调用调度块对象（dispatch block object）时，此标志无效。
 *
 ...
 * 标志 应该执行与当前执行上下文属性（例如 os_activity_t 和当前 IPC 请求的属性，如果有）无关的调度块对象（dispatch block object。
 * 关于 QoS 类别，其行为与 DISPATCH_BLOCK_NO_QOS 相同。
 * 如果直接调用，则块对象将在块主体的持续时间内从调用线程中删除其他属性（在应用分配给块对象的属性 （如果有）之前）。
 * 如果提交给队列，则将使用队列的属性（或专门分配给该块对象的任何属性）执行该块对象。
 *
 ...
 * 标识 应为调度块对象分配创建块对象时当前的执行上下文属性。
 * 这适用于诸如 QOS 类，os_activity_t 的属性以及当前 IPC 请求的属性（如果有）。
 * 如果直接调用，则块对象将在块主体的持续时间内将这些属性应用于调用线程。
 * 如果将块对象提交到队列，则此标志替换将提交的块实例与提交时最新的执行上下文属性相关联的默认行为。
 * 如果使用 DISPATCH_BLOCK_NO_QOS_CLASS 或 dispatch_block_create_with_qos_class 分配了特定的 QOS 类，则该 QOS 类优先于此标志指示的 QOS 类分配。
 ...
 * 指示不应为调度块对象分配 QOS 类的标志。
 * 如果直接调用，则块对象将与调用线程的 QOS 类一起执行。
 * 如果将块对象提交到队列，这将替换默认行为，即在提交时将提交的块实例与当前的 QOS 类相关联。
 * 如果为特定的 QOS 类分配了 dispatch_block_create_with_qos_class，则忽略此标志。
 ...
 * also passed.
 * 指示执行提交到队列的调度块对象的标志应优先于分配给队列的 QOS 类，而不是分配给该块的 QOS 类（提交时与该块相关联的 resp。）。
 * 仅当所讨论的队列没有分配的 QOS 类时，才使用后者，只要这样做不会导致 QOS 类低于从队列的目标队列继承的 QOS 类。
 * 当将调度块对象提交到队列以进行异步执行时，此标志是默认设置；
 * 当直接调用调度块对象时，此标志无效。如果还传递了 DISPATCH_BLOCK_ENFORCE_QOS_CLASS，则将其忽略。
 *
 ...
 * directly.
 * 指示执行提交到队列的调度块对象的标志应优先于分配给队列的 QOS 类，而不是分配给队列的 QOS 类，分配给该块的 QOS 类（在提交时与该块相关联）不会导致较低的 QOS 等级。
 ...
 * the given flags.
 * 根据现有块（existing block）和给定的标志（flags）在堆（heap）上创建一个新的调度块对象（dispatch block object）。
 *
 * @discussion
 * The provided block is Block_copy'ed to the heap and retained by the newly
 * created dispatch block object.
 * 提供的块被 Block_copy 到堆中，并由新创建的调度块对象保留。
 *
 ...
 * 块对象（block object）的配置标志。传递的值与 dispatch_block_flags_t 中的标志不是按位或的结果将返回 NULL。
 *
 * @param block
 * The block to create the dispatch block object from.
 * 从中创建调度块对象（dispatch block object）的块。
 *
 * @result
 ...
 * 新创建的调度块对象，或者 NULL。当不使用 Objective-C ARC 构建时，必须使用 -[release] 消息或 Block_release 函数来进行释放。
 */
 ...
 * 从指定的块（block）和标志（flags）创建，同步执行（synchronously execute）和释放（release）调度块对象（dispatch block object）。
 *
 * @discussion
 ...
 * 临时块对象的配置标志。传递与 dispatch_block_flags_t 中的标志不是按位或的值，结果将是未定义的。
 *
 * @param block
 ...
 * code never runs.
 * 不能使用此接口等待同一块对象的多次执行；为此，请使用 dispatch_group_wait()。
 * 单个调度块对象（dispatch block object）可以等待一次并执行一次， 也可以执行任意次数 。
 * 任何其他组合的行为都是未定义的。
 * 即使取消（dispatch_block_cancel）表示该块的代码从不运行，向调度队列的提交也被视为执行。
 ...
 * 如果此函数的返回值指示已超过指定的超时，则该调用不算作允许的一次等待。
 *
 ...
 * 如果在调用此函数时，指定的调度块对象已直接提交到串行队列，则系统将尽最大努力应用必要的 QOS 覆盖，以确保该块和任何较早提交给该串行队列的块在调用 dispatch_block_wait 的线程的 QOS 类（或更高版本）上执行。
 *
 * @param block
 ...
 * 要等待的调度块对象。
 * 传递 NULL 或未由 dispatch_block_create* 函数之一返回的块对象的结果是未定义的。
 *
 * @param timeout
 ...
 * 计划在完成指定调度块对象（block）的执行后将通知块（notification_block）提交给队列。
 *
 * @discussion
 ...
 * 单个分发块对象（single dispatch block object）可以被观察一次或多次并执行一次，也可以执行任意次。
 * 任何其他组合的行为均未定义。
 * 即使取消（dispatch_block_cancel）表示该块的代码从不运行，向调度队列的提交也被视为执行。
 *
 ...
 * 异步取消指定的调度块对象（dispatch block object）。
 *
 * @discussion
 ...
 * 注意：需要注意确保可以取消的块对象不会捕获需要执行块体才能释放的任何资源（例如，块体调用 free（用 malloc 分配的内存））。
 * 如果由于取消而从不执行块体，则这些资源将被泄漏。
 *
 * @param block
 * The dispatch block object to cancel.
 * The result of passing NULL or a block object not returned by one of the
 * dispatch_block_create* functions is undefined.
 * 要取消的调度块对象。传递 NULL 或未由 dispatch_block_create* 函数之一返回的块对象的结果是不确定的。
 */
API_AVAILABLE(macos(10.10), ios(8.0))
DISPATCH_EXPORT DISPATCH_NONNULL_ALL DISPATCH_NOTHROW
void
 ...
 * 测试给定的调度块对象（dispatch block object）是否已取消。
 *
 * @param block
