 ...
 * dispatch framework 提供了一套接口，用于监视低级系统对象（file descriptors（文件描述符）, Mach ports, signals, VFS nodes, 等.）的活动，并在此类活动发生时自动向 dispatch queues 提交事件处理程序块（event handler blocks）。
 *
 * This suite of interfaces is known as the Dispatch Source API.
 * 这套接口称为 Dispatch Source API。
 */
 ...
 * interpreted.
 * 此类型的常量表示调度源（dispatch source）正在监视的低级系统对象的类（class of low-level system object）。此类型的常量作为参数传递给 dispatch_source_create 函数并确定如何解释 handle 参数（handle argument ）（i.e. as a file descriptor（文件描述符）, mach port, signal number, process identifier, etc.）以及如何解释 mask 参数（mask argument）。
 */
 ...
 * 一种调度源（dispatch source），它合并通过调用 dispatch_source_merge_data() 获得的数据。ADD 用于合并数据。
 * 句柄未使用（暂时传递零），mask 未使用（暂时传递零）。
 */
 ...
 * 一种调度源（dispatch source），它跟踪通过调用 dispatch_source_merge_data 获得的数据。新获得的数据值替换了尚未传递到源处理程序（source handler）的现有数据值。
 *
 * A data value of zero will cause the source handler to not be invoked.
 * 数据值为零将不调用源处理程序（source handler）。
 *
 * The handle is unused (pass zero for now).
 * The mask is unused (pass zero for now).
 */
 ...
 * 一种调度源（dispatch source），用于监视 Mach port 的 dead name 通知（发送权限不再具有任何相应的接收权限）。
 * 句柄（handle）是一个 Mach port，具有 send 或 send once right（mach_port_t）。
 * mask 是 dispatch_source_mach_send_flags_t 中所需事件的 mask。
 *
 */
 ...
 *  一种调度源（dispatch source），用于监视系统内存压力状况的变化。
 *  该句柄（handle）未使用（现在传递零）。
 *  mask 是来自 dispatch_source_mach_recv_flags_t 中所需事件的 mask。
 *
 */
 ...
 * 一种调度源（dispatch source），用于监视外部进程中由 dispatch_source_proc_flags_t 定义的事件。
 * 句柄（handle）是进程标识符（pid_t）。
 * mask 是来自 dispatch_source_mach_recv_flags_t 中所需事件的 mask。
 */
 ...
 * The mask is unused (pass zero for now).
 * 一种调度源（dispatch source），用于监视文件描述符的待处理字节，以获取可读取的字节。
 * 句柄（handle）是文件描述符（int）。
 * mask 未使用（现在传递零）。
 */
 ...
 * The mask is unused (pass zero for now).
 * 监视当前进程以获取信号的调度源（dispatch source）。
 * 句柄（handle）是信号编号（int）。
 * mask 未使用（现在传递零）。
 */
 ...
 * 基于计时器（based on a timer）提交（submits）事件处理程序块（event handler block）的调度源（dispatch source）。
 * 句柄（handle）未使用（现在传递零）。
 * mask 指定要应用的来自 dispatch_source_timer_flags_t 的标志。
 */
 ...
 * 一种调度源（dispatch source），它监视由 dispatch_source_vnode_flags_t 定义的事件的文件描述符。
 * 句柄（handle）是文件描述符（int）。
 * mask 是来自 dispatch_source_vnode_flags_t 的所需事件的 mask。
 */
 ...
 * The handle is a file descriptor (int).
 * The mask is unused (pass zero for now).
 * 一种调度源（dispatch source），它监视文件描述符以获取可用于写入字节的缓冲区空间。
 * 句柄（handle）是文件描述符（int）。
 * mask 未使用（现在传递零）。
 */
 ...
 * pressure.
 * 内存压力升高是一种系统范围内的情况，为此源注册的应用程序应通过更改其将来的内存使用行为来作出反应，
 * 例如：通过减少新启动操作的缓存大小，直到内存压力恢复正常。
 ...
 * exec*() or posix_spawn*().
 * 通过 exec *() 或 posix_spawn *()，该进程已成为另一个可执行映像（executable image）。
 *
 ...
 * DISPATCH_TIMER_STRICT 指定系统应尽最大努力严格遵守通过 dispatch_source_set_timer() 为计时器指定的leeway value，即使该值小于将应用于计时器的默认leeway value。
 * 即使指定了此标志，也会对计时器应用最小量的leeway value。
 *
 ...
 * Dispatch sources 不可重入。
 * 在调度源被挂起或事件处理程序块当前正在执行时，
 * 接收到的任何事件都将在调度源恢复或事件处理程序块返回后合并和传递。
 *
 ...
 * Dispatch sources 在非活动状态下创建。
 * 创建源并设置任何所需的属性（即处理程序，上下文等）之后，必须调用 dispatch_activate 才能开始事件传递。
 *
 ...
 * 声明调度源的类型。必须是已定义的 dispatch_source_type_t 常量之一。
 *
 * @param handle
 ...
 * 要监视的基础系统句柄（handle）。此参数的解释由 type 参数中提供的常量确定。
 *
 * @param mask
 ...
 * 指定所需事件的标志 mask。此参数的解释由 type 参数中提供的常量确定。
 *
 * @param queue
 ...
 * 事件处理程序块（event handler block）将提交到的调度队列。如果队列为 DISPATCH_TARGET_QUEUE_DEFAULT，则源（source）将事件处理程序块提交到默认优先级全局队列。
 *
 * @result
 * The newly created dispatch source. Or NULL if invalid arguments are passed.
 * 新创建的调度源（dispatch source）。如果传递了无效的参数，则为 NULL。
 */
API_AVAILABLE(macos(10.6), ios(4.0))
 ...
 * 要进行修改的调度源。在此参数中传递 NULL 的结果是未定义的
 *
 * @param handler
 ...
 * 事件处理程序函数提交到源的目标队列。传递给事件处理程序（函数）的 context 参数是设置事件处理程序时当前调度源的上下文。
 */
API_AVAILABLE(macos(10.6), ios(4.0))
DISPATCH_EXPORT DISPATCH_NONNULL1 DISPATCH_NOTHROW
void
 ...
 * 为给定的调度源设置取消处理程序块（cancellation handler block）。
 *
 * @discussion
 ...
 * 一旦系统释放了对源基础句柄的所有引用，并且返回了源的事件处理程序块，
 * 则取消处理程序（如果已指定）将被提交到源的目标队列，以响应对 dispatch_source_cancel 的调用。
 *
 * IMPORTANT:
 ...
 * file descriptor 和基于 mach port 的源需要源取消（source cancellation）和取消处理程序（a cancellation handler），以便安全地关闭描述符或销毁端口。
 * 在调用取消处理程序之前关闭描述符或端口可能会导致竞态。
 * 如果在源的事件处理程序仍在运行时为新描述符分配了与最近关闭的描述符相同的值，
 * 则事件处理程序可能会将数据读/写到错误的描述符。
 *
 * @param source
 * The dispatch source to modify.
 * The result of passing NULL in this parameter is undefined.
 ...
 * 异步取消调度源（dispatch source），以防止进一步调用其事件处理程序块。
 *
 * @discussion
 ...
 * 返回与此调度源关联的基础系统句柄（underlying system handle）。
 *
 * @param source
 * The result of passing NULL in this parameter is undefined.
 *
 ...
 *
 * 将数据合并到类型为 DISPATCH_SOURCE_TYPE_DATA_ADD，DISPATCH_SOURCE_TYPE_DATA_OR 或 DISPATCH_SOURCE_TYPE_DATA_REPLACE 的调度源中，并将其事件处理程序块提交到其目标队列。
 ...
 * 使用调度源类型指定的逻辑 OR 或 ADD 与待处理数据合并的值。
 * 零值无效并且也不会导致事件处理程序块的提交。
 */
API_AVAILABLE(macos(10.6), ios(4.0))
DISPATCH_EXPORT DISPATCH_NONNULL_ALL DISPATCH_NOTHROW
void
 ...
 * 设置 timer source 的开始时间，间隔和回程值（leeway value）。
 *
 * @discussion
 ...
 * 此函数返回后，将清除先前计时器值累积的所有未决源数据；
 * 计时器的下一次触发将在 start 时发生，此后每隔 interval 纳秒，直到计时器源被取消。
 *
 ...
 * 系统可能会延迟计时器的任何触发时间，以改善功耗和系统性能。允许延迟的上限可以使用 leeway 参数进行配置，下限在系统的控制下。
 *
 ...
 * 对于 start 时的初始计时器触发，允许延迟的上限设置为 leeway 纳秒。
 * 对于随后的计时器以 start + N * interval 触发的情况，上限为 MIN（leeway，interval / 2）。
 *
 ...
 * 允许延迟的下限可能随过程状态（例如应用程序 UI 的可见性）而变化。如果指定的计时器源是使用 DISPATCH_TIMER_STRICT 的 mask 创建的，则系统将尽最大努力严格遵守所提供的 leeway 值，即使该值小于当前下限。请注意，即使指定了此标志，也希望有最小的延迟量。
 *
 ...
 * start 参数还确定将使用哪个时钟作为计时器：如果 start 是 DISPATCH_TIME_NOW 或由 dispatch_time(3) 创建的，则计时器基于正常运行时间（从 Apple 平台上的 mach_absolute_time 获取） 。如果使用 dispatch_walltime(3) 创建了 start，则计时器基于 gettimeofday(3)。
 *
 * Calling this function has no effect if the timer source has already been
 * canceled.
 * 如果 timer source 已被取消，则调用此函数无效。
 *
 * @param start
 * The start time of the timer. See dispatch_time() and dispatch_walltime()
 * for more information.
 * 计时器的开始时间。参考 dispatch_time() 和 dispatch_walltime()。
 *
 * @param interval
 * The nanosecond interval for the timer. Use DISPATCH_TIME_FOREVER for a
 * one-shot timer.
 * 计时器的纳秒间隔。将 DISPATCH_TIME_FOREVER 用于一键式计时器（a one-shot timer）。
 *
 * @param leeway
 ...
 * 设置给定调度源的注册处理程序块（registration handler block）。
 *
 * @discussion
 ...
 * 设置给定调度源（dispatch source）的注册处理函数。
 *
 * @discussion
